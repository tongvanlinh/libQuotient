...
//!          render a RoomMember dangling; calling any of its methods will then lead to undefined
//!          behaviour.
//! \sa User
class QUOTIENT_API MemberProxy {
    Q_GADGET
    QML_NAMED_ELEMENT(memberProxy)
    QML_UNCREATABLE("")
...
public:
    ...
    explicit MemberProxy(const Room* room, const RoomMemberEvent* member);

    ~MemberProxy();

    bool isEmpty() const { return _member == nullptr; }

    // The reason it is deprecated is that the expected comparison semantics are ambiguous: the two
    // objects might be equal because they merely refer to the same room member; or they refer to
    // the same _state_ of the same room member
    [[deprecated("This operator only compares member ids which may be surprising in the code. "
                 "Compare member ids and, if you need, events directly in your code instead")]]
    bool operator==(const MemberProxy& other) const;
...
struct QUOTIENT_API MemberSorter {
    ...
    bool operator()(const RoomMemberEvent *e1, const RoomMemberEvent *e2) const;
    bool operator()(const MemberProxy& m1, const MemberProxy& m2) const
    {
        return operator()(m1.displayName(), m2.displayName());
    }
};
