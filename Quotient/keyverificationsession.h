...
public:
    enum State {
        ...
        DECL_DEPRECATED_ENUMERATOR(INCOMING, REQUESTED),
        READY, //! The verification request is ready to start a verification flow
        TRANSITIONED, //! The verification request has transitioned into a concrete verification flow, e.g. into the emoji based SAS verification
        ...
        CANCELED = CANCELLED,
        NOTFOUND, //! The verification session was not found
    };
    Q_ENUM(State)

    enum SasState {
        SASSTARTED, //! The verification has been started, the protocols that should be used have been proposed and can be accepted
        SASACCEPTED, //! The verification has been accepted and both sides agreed to a set of protocols that will be used for the verification process
        SASKEYSEXCHANGED, //! The public keys have been exchanged and the short auth string can be presented to the user
        SASCONFIRMED, //! The verification process has been confirmed from our side, we’re waiting for the other side to confirm as well
        SASDONE, //! The verification process has been successfully concluded
        SASCANCELLED, //! The verification process has been cancelled
        SASNOTFOUND, //! The verification session is not found / not transitioned to SAS
    };
    Q_ENUM(SasState)
    ...
    Q_PROPERTY(QList<EmojiEntry> sasEmojis READ sasEmojis NOTIFY sasEmojisChanged)
    ...
    static KeyVerificationSession* selfVerification(const QString& verificationId, Connection* connection);
